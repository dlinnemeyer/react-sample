...
export function addItem(item){
  return (dispatch, getStore) => {
    return goAddItem(item, getStore())
      .then(item => {
...
export function deleteConsignor(consignor){
  return (dispatch, getStore) => {
    return goDeleteConsignor(consignor)
      .then(consignor => {
...
export function deleteConsignorLoading(isLoading){
  return {
    type: 'DELETE_CONSIGNOR_LOADING',
    isLoading
  }
}
...
export function deleteItemLoading(isLoading){
  return {
...
// another "ajax" call
function goAddItem(item, store){
  if(!item.id) item.id = (Math.floor(Math.random() * 1000000)) + "";

  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const skus = Object.keys(store.items).map(id => store.items[id].sku);
...
// another "ajax" call
function goDeleteConsignor(consignor){
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      let err = {};
      if(err = randomNetworkError()){
